...
@if ($component->getModel()->hasContainee('items'))
    ...
    @foreach ($component->getModel()->getContainees('items') as $item)
        <li id="{{ $item->getModelViewerComponent()->getDomId('list-containee', md5(get_class($item)), $item->getKey()) }}" data-containee-id="{{ $item->getKey() }}" data-containee-type="{{ get_class($item) }}" class="col-xxl-3 col-xl-4 col-lg-6 col-xs-12 height-150">
            <div class="row">
