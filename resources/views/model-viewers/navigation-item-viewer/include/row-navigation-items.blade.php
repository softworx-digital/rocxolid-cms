    ...
    <div class="row">
        <div class="col-xs-2 text-left actions">
            <div class="btn-group">
                ...
                <a type="button" class="btn btn-primary btn-sm margin-right-no" title="{{ $component->translate('table-button.edit') }}" data-ajax-url="{{ $component->getModel()->getControllerRoute('edit', [ '_section' => 'row-navigation-items', '_data[container_id]' => $container->id, '_data[container_type]' => get_class($container), '_data[container_relation]' => 'items' ]) }}"><i class="fa fa-pencil"></i></a>
                <a type="button" class="btn btn-danger btn-sm margin-right-no"  title="{{ $component->translate('table-button.detach') }}" data-ajax-url="{{ $component->getModel()->getControllerRoute('detach', [ '_section' => 'row-navigation-items', '_data[container_id]' => $container->id, '_data[container_type]' => get_class($container), '_data[container_relation]' => 'items' ]) }}"><i class="fa fa-minus"></i></a>
            </div>
        </div>
        <div class="col-xs-10">
